...
**Status:** Docs + labels + skills + eligibility landed (#204–#207); `/planner` teaching surface (skill invoke `/vilya-plan`) landed in #208; Setup/differences model-routing rewrite remains #209; skill rename canonized in #257/#260; intake Monitor (#255) amends idle/monitor clause from #203; #261 makes orchestrator standing `plan:ready` poller the required completion wake (per-enqueue reinforcement); #267 amends lifecycle to persist one poller across drains; Cursor monitor-shell teardown/re-arm taught in #270

...
routing are owned by `/vilya-start-feature`, not by this skill:

1. **`/vilya-start-feature #<n>`** — issue, worktree, branch, In Progress, verify plan on the issue.
   Branches are **daytime-style** `feat|fix|docs/<issue#>-slug` under `.claude/worktrees/`
   (Actions `_work` checkout). **Not** chip `claude/*` branches — `/vilya-prune` expects that pairing.
2. **Implement** in the owning vertical slice (VSA + SOLID bar = daytime).
3. **`/vilya-crucible-<stack>`** from config (`vilya-crucible-blazor` | `vilya-crucible-nextjs`): apply top
   remediations, re-review until **Ready** — cap at **3 refactor rounds**, then stop and report.
4. **`/vilya-finish-feature`** — tests green, changelog fragment, open the PR.
5. **Detach the worktree** before the next issue — from the Actions checkout root (not inside the
   ...
   **remote** branch + open PR alone. Self-hosted `_work` persists between runs; do not leave
   trees piled up for morning `/vilya-prune` unless remove fails (then report the path).
...
see [/vilya-chip](../vilya-chip/SKILL.md) §2a.)
...
- **Never merge, never force-push a shared branch, never push to the default branch.** You open the
  PR; the operator merges in the morning (via `/vilya-merge-pr`).
- **Morning review queue, not chip-style as-they-open review.** Chip PRs are reviewed when each
  ...
  list — operator `/vilya-merge-pr` (or reject) after waking. Do not assume anyone watched the run.
...
- Bugs found mid-work → new linked Bug issue (`/vilya-update-docs`), don't derail the current one.

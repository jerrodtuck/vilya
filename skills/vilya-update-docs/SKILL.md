...
> Companions: [/vilya-start-feature](../vilya-start-feature/SKILL.md) · [/vilya-finish-feature](../vilya-finish-feature/SKILL.md).

**Routing skill** — invoke when something needs placing. It is **not** part of the happy path
(`/vilya-start-feature` → crucible → `/vilya-finish-feature` → `/vilya-merge-pr`). Specs and changelog fragments on
...
| Design decision | `docs/DECISIONS.md` — reference from issue | run [/vilya-adr](../vilya-adr/SKILL.md) — it captures + mirrors |
...
| Append-only | `changelog.d/YYYY-MM-DD-<slug>.md`, `DECISIONS.md` | Changelog: dated filename. Decisions: newest-at-top `## YYYY-MM-DD — Title`; entry shape owned by [/vilya-adr](../vilya-adr/SKILL.md). |

...
description: >-
  ...
  crucible → /vilya-finish-feature. Does not seat the orchestrator, open folders, or
  teach a three-step paste ritual. Name is /vilya-cursor-handoff — not /chip-cursor.
  Pairs with /vilya-start-feature, /vilya-finish-feature, /vilya-prune; Claude Code chips use /vilya-chip
  instead.
---
...
> Companion: [/vilya-start-feature](../vilya-start-feature/SKILL.md) already ran in the
...
> [/vilya-finish-feature](../vilya-finish-feature/SKILL.md). [/vilya-prune](../vilya-prune/SKILL.md) is
> orchestrator-owned from the main clone. Claude Code chips use
> [/vilya-chip](../vilya-chip/SKILL.md) + `spawn_task`. Repo / owner / project / labels /
...
- There **must be a tracking issue** with an orchestrator kickoff.
  [/vilya-start-feature](../vilya-start-feature/SKILL.md) **already ran** — **do not re-run
...
1. Run the repo's `vilya-crucible-<stack>` (from `GITHUB-PROJECTS.md`) on the branch;
   apply top refactors; re-review until the signal reads **Ready**. This gate is
   not optional.
2. Close with [/vilya-finish-feature](../vilya-finish-feature/SKILL.md) — PR titled

// #183: node click highlights every flow through that node. That only works
// if the data holds two invariants: every node on the map belongs to at least
// one named flow, and every flow references only real node/edge ids.
...
describe("orchestrator flow coverage", () => {
  ...
  it("flows reference only real node and edge ids", () => {
    ...
    for (const [id, f] of namedFlows) {
      for (const n of f.nodes ?? []) {
        expect(nodeIds.has(n), `flow ${id} references unknown node ${n}`).toBe(true);
      }
      for (const e of f.edges ?? []) {
        expect(edgeIds.has(e), `flow ${id} references unknown edge ${e}`).toBe(true);
      }
    }
  });

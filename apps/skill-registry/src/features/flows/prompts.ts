...
A chip's send_message report is the primary completion signal — it flows unattended only once mcp__ccd_session_mgmt__send_message is allowed in user-level settings (one-time setup). Polling is the verification backup: check list_sessions (prState/isRunning) or gh pr list whenever a report hasn't arrived, and always verify before merge — a report is a claim, not proof. Then review that chip's commits.

Your jobs: board/issue ops; kickoff plans; writing self-contained chip briefs with verify gates; receiving chip reports, verifying them, and reviewing each chip's PR; merging reviewed chips via /merge-pr (squash, never delete the branch); worktree cleanup via /prune. House rules: vertical-slice architecture, outcome-oriented SOLID; one issue = one branch. Track all new work as GitHub issues on the board — never markdown trackers. At any real design fork, stop and give me 2–3 options with costs in plain chat text before any chip is dispatched. Hold the crucible review bar and report progress honestly.`,
      },
      {

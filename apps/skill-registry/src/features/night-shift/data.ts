...
export const STAGES: Record<StageId, NightStage> = {
  ...
  LOOP: {
    ...
    the night-shift <b>Skill</b> (<code>skills/vl-night-shift/SKILL.md</code>) — same daytime chain, leave a PR.</p>
    <ul>
  ...
  STEERING: {
    ...
    chipLabel: "Steering",
    bodyHtml: `
    <p>The night-shift <b>Skill</b> (<code>skills/vl-night-shift/SKILL.md</code>) is not a second methodology — it runs the
    ...
    <ul>
      ...
      <li><b>Daytime chain</b> — <code>/vl-start-feature</code> → implement → <code>/vl-crucible-&lt;stack&gt;</code> (≤3 rounds to Ready) → <code>/vl-finish-feature</code> → detach worktree.</li>
      <li><b>Branches</b> — daytime <code>feat|fix|docs/&lt;issue#&gt;-*</code> under <code>.claude/worktrees/</code> (Actions <code>_work</code>), <b>not</b> chip <code>claude/*</code>. <code>/vl-prune</code> owns that pairing.</li>
      <li><b>Fork stop</b> — comment + recommendation, label <code>needs:decision</code>, Blocked, next issue. Never guess.</li>
      <li><b>PR never merge</b> — open only; operator merges via <code>/vl-merge-pr</code> in the morning.</li>

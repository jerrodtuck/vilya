...
describe("night-agent stage model", () => {
  ...
  it("Steering and Outputs distinguish night-shift branches from chips", () => {
    ...
    expect(STAGES.STEERING.bodyHtml).toContain("/vl-prune");
  ...
  it("Steering keeps the skill dumb; Outputs stay artifacts-only (#200)", () => {
    ...
    expect(STAGES.OUTPUTS.bodyHtml).not.toContain("/vl-merge-pr");
    expect(STAGES.OUTPUTS.bodyHtml).not.toContain("/vl-prune");
  });

...
description: >-
  ...
  /vilya-prune. Use when the user says "merge PR #N", "review and merge", "process the
  review queue", or the morning after /vilya-night-shift leaves PRs. Pairs with
  /vilya-finish-feature and /vilya-prune.
---

# Merge PR (any stack)

> Companion: [/vilya-finish-feature](../vilya-finish-feature/SKILL.md) opens PRs; [/vilya-night-shift](../vilya-night-shift/SKILL.md)
> leaves them overnight **unreviewed** (morning batch — not chip as-they-open review);
> [/vilya-prune](../vilya-prune/SKILL.md) cleans Cursor + Claude (chip `claude/*` and night-shift
...
- **Manual test owed** — the issue's verify plan declared `local-smoke`, or a UI/behavioral
  change has thin automated coverage. The routing was decided at kickoff (`/vilya-start-feature`
...
Report exact counts, same bar as `/vilya-finish-feature` step 1.
...
worktree from `/vilya-start-feature` is **not** deleted here — that is [/vilya-prune](../vilya-prune/SKILL.md).
...
Send back through `/vilya-finish-feature` step 2 (rebase + re-verify on the feature branch), then
re-triage here, when:
...
branch **and** worktree are `/vilya-prune` (§5). That split stops merge-pr from deleting a branch
...
- On 405 / conflicts → §3 (`/vilya-finish-feature` rebase), not a retry without rebase.
...
- **Cleanup is a handoff, not an in-place delete:** tell the operator to run `/vilya-prune` (dry-run)
  then `/vilya-prune --apply` from the **main clone** (and, after a night-shift batch, from the
  ...
  you are standing in. Cursor Archive / Claude delete do **not** replace `/vilya-prune` for
  `%USERPROFILE%\.cursor\worktrees\<repo>\<issue#>-*` or `.claude/worktrees/` paths. If prune
  hits Permission denied on an eligible row, see [/vilya-prune](../vilya-prune/SKILL.md) §5a —
  leftover `cursor-agent-worker` may hold `--worker-dir`; `/vilya-prune --apply` kills matching

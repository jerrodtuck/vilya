...
description: >-
  ...
  crucible → /finish-feature. Does not seat the orchestrator, open folders, or
  teach a three-step paste ritual. Name is /cursor-handoff — not /chip-cursor.
  Pairs with /start-feature, /finish-feature, /prune; Claude Code chips use /chip
  instead.
---
...
> Companion: [/start-feature](../start-feature/SKILL.md) already ran in the
...
> [/finish-feature](../finish-feature/SKILL.md). [/prune](../prune/SKILL.md) is
> orchestrator-owned from the main clone. Claude Code chips use
> [/chip](../chip/SKILL.md) + `spawn_task`. Repo / owner / project / labels /
...
- There **must be a tracking issue** with an orchestrator kickoff.
  [/start-feature](../start-feature/SKILL.md) **already ran** — **do not re-run
...
1. Run the repo's `crucible-<stack>` (from `GITHUB-PROJECTS.md`) on the branch;
   apply top refactors; re-review until the signal reads **Ready**. This gate is
   not optional.
2. Close with [/finish-feature](../finish-feature/SKILL.md) — PR titled

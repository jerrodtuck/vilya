...
description: >-
  ...
  do the work and open their own PRs. Pairs with /vilya-start-feature, /vilya-merge-pr, /vilya-prune.
---

# Chip (any stack)

> Companion: [/vilya-start-feature](../vilya-start-feature/SKILL.md) creates issues/board + the plan; chips
> do the implementation; [/vilya-merge-pr](../vilya-merge-pr/SKILL.md) merges reviewed chips; [/vilya-prune](../vilya-prune/SKILL.md)
...
- There **must be a tracking issue** — create it via [/vilya-start-feature](../vilya-start-feature/SKILL.md)
  or [/vilya-update-docs](../vilya-update-docs/SKILL.md) first. **Never chip untracked work.**
...
- **Crucible gate**: run the repo's `vilya-crucible-<stack>` skill (looked up in `GITHUB-PROJECTS.md`,
  e.g. `vilya-crucible-blazor` / `vilya-crucible-nextjs`) and remediate until the signal reads **Ready** —
  **not optional**.
- **Close-out**: **`/vilya-finish-feature`** (not a hand-rolled PR) — after the crucible gate above reads
  **Ready**, it opens the PR **titled `#<N> <name>`** with **`Closes #<N>`**, plus the `changelog.d/`
  fragment + spec status.
- **Completion report**: right after `/vilya-finish-feature` opens the PR, post a concise
...
| **Unattended / night-shift** | Label **`needs:decision`**, Status **Blocked** | Same options comment + recommendation; do **not** wait — take the next eligible issue (`/vilya-night-shift`). |
...
- Merge reviewed chips with **[/vilya-merge-pr](../vilya-merge-pr/SKILL.md)** — squash; it does **not** delete
  the branch.
- Worktree + branch cleanup is **[/vilya-prune](../vilya-prune/SKILL.md)**, from the main clone after merge. If a
  chip worktree is locked (**Permission denied**), **close the chip session in the UI** to release
  it, then `/vilya-prune --apply`.
...
- Chips **never self-merge**; the orchestrator reviews every chip before `/vilya-merge-pr`.

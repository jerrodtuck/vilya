---
name: vilya-start-feature
description: Kick off a new feature or work stream on any repo's GitHub Projects board. Use when the user says "start on <feature/brief>", "pick up issue #N", "begin the <X> work", "work the next thing", or points at an issue. Sets up the issue, branch, and consult-first flow; pairs with /vilya-finish-feature.
---
...
- Ambiguous which issue → ask (unless invoked by [/vilya-night-shift](../vilya-night-shift/SKILL.md) —
...
5. **Execute phase** runs in the chip/worker only after the plan is settled (or after the
   ...
   `/vilya-night-shift`, not rewritten here.
...
- **New defect mid-work** → new Bug issue (`/vilya-update-docs`), link it, keep going.
...
- **Crucible is mandatory before finish:** after implementation and green tests, run the stack
  crucible skill (`/vilya-crucible-blazor` for .NET/Blazor, `/vilya-crucible-nextjs` for Next.js) — not a
  verbal "hold the bar." Apply 🔴/🟠 remediations until merge-readiness is `Ready` (or
  `Ready after blockers` with blockers fixed). Then [/vilya-finish-feature](../vilya-finish-feature/SKILL.md).
...
- **Merge routing** — one of:
  ...
  - `local-smoke` — a hands-on check is owed but runs locally (launch the app, drive the flow);
    done **pre-merge** via `/vilya-merge-pr`; PR still uses `Closes #`.
...
investigate-first pick is recorded). Close path: **tests green → `/vilya-crucible-<stack>` → remediate →
`/vilya-finish-feature` → operator merges via `/vilya-merge-pr`**.

...
description: >-
  ...
  "prune", "prune worktrees", "/vilya-prune", or after /vilya-merge-pr hands off cleanup. Run from the
  main clone — never from inside a worktree being removed.
---

# Prune (any stack)

> Companion: [/vilya-merge-pr](../vilya-merge-pr/SKILL.md) squash-merges and **hands off** here;
> [/vilya-start-feature](../vilya-start-feature/SKILL.md) creates the worktrees this skill removes;
> [/vilya-night-shift](../vilya-night-shift/SKILL.md) uses the same `feat|fix|docs/*` branch names under
...
```text
%USERPROFILE%\.cursor\worktrees\<repo>\<issue#>-<slug>   # Cursor daytime (/vilya-start-feature)
...
| **Night-shift** (and Claude daytime `/vilya-start-feature`) | `.claude/worktrees/` (incl. Actions `_work`) | `feat\|fix\|docs/<issue#>-*` |
...
| Prompt | Effect |
|--------|--------|
| `/vilya-prune` · "prune worktrees" · "what can we prune?" | **Dry-run** — list eligible paths + branches, touch nothing; preview `would kill PID …` when a lock holder is detected |
| `/vilya-prune --apply` · "prune worktrees, apply" | Remove what the dry-run would list; **`--apply` authorizes scoped lock-holder kills** on eligible rows (see §5a) |
...
- One issue: `/vilya-prune 36` / `/vilya-prune --apply 36`
...
2. **`git worktree list --porcelain` from each relevant checkout is the source of truth** —
   ...
   - **Cursor daytime** (`/vilya-start-feature`):
   ...
   - **Night-shift / Claude daytime** (`/vilya-start-feature` under Claude):
...
End with counts: `N eligible · M skipped · apply with /vilya-prune --apply`.
...
After `/vilya-merge-pr`, folder delete can return **Permission denied** even though the PR is
...
- Never kill processes that do not name the target worktree; never skip eligibility to
  force-delete.
- After `/vilya-merge-pr`, one `/vilya-prune --apply` (or a dry-run the operator reviews) is the
